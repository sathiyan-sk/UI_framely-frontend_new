...
const ANIM = `
  ...
  @keyframes zipBounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}
`

// ─── ZIP Download Modal ───────────────────────────────────────────────────────
function ZipDownloadModal({ stage, count, total, onClose }) {
  const stages = [
    { key: 'collecting', icon: '📂', label: 'Collecting your photos', sub: `Gathering ${total} photos from the cloud` },
    { key: 'compressing', icon: '🗜️', label: 'Compressing photos', sub: 'Creating your ZIP file, this may take a moment' },
    { key: 'downloading', icon: '⬇️', label: 'Starting download', sub: 'Your browser will save the file shortly' },
    { key: 'done', icon: '✅', label: 'Download complete!', sub: 'Your photos are saved to your downloads folder' },
  ]
  const currentIdx = stages.findIndex(s => s.key === stage)
  const progress = stage === 'done' ? 100 : stage === 'downloading' ? 85 : stage === 'compressing' ? 55 : 20

  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(13,13,26,0.85)', backdropFilter: 'blur(20px)', fontFamily: "'Inter',system-ui,sans-serif" }}>
      <style>{ANIM}</style>
      <div style={{ background: 'rgba(255,255,255,0.96)', borderRadius: 28, padding: '36px 40px', maxWidth: 420, width: '90%', boxShadow: '0 32px 80px rgba(45,27,105,0.25)', textAlign: 'center' }}>

        {/* Animated icon */}
        <div style={{ fontSize: 56, marginBottom: 16, animation: stage !== 'done' ? 'zipBounce 1.2s ease-in-out infinite' : 'zoomIn .3s ease' }}>
          {stages[currentIdx]?.icon || '📦'}
        </div>

        <div style={{ fontSize: 18, fontWeight: 800, color: '#2d1b69', marginBottom: 6 }}>
          {stages[currentIdx]?.label}
        </div>
        <div style={{ fontSize: 13, color: '#9b89c4', marginBottom: 24, lineHeight: 1.5 }}>
          {stages[currentIdx]?.sub}
        </div>

        {/* Progress bar */}
        <div style={{ height: 6, background: 'rgba(155,127,232,0.12)', borderRadius: 6, overflow: 'hidden', marginBottom: 20 }}>
          <div style={{ height: '100%', width: `${progress}%`, background: 'linear-gradient(90deg,#9b7fe8,#c084fc)', borderRadius: 6, transition: 'width 0.8s cubic-bezier(.4,0,.2,1)' }} />
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24 }}>
          {stages.map((s, i) => (
            <div key={s.key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, flex: 1 }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, background: i < currentIdx ? 'linear-gradient(135deg,#9b7fe8,#c084fc)' : i === currentIdx ? 'rgba(155,127,232,0.15)' : 'rgba(0,0,0,0.05)', color: i <= currentIdx ? (i < currentIdx ? '#fff' : '#7c3aed') : '#c4b5fd', border: i === currentIdx ? '2px solid #9b7fe8' : '2px solid transparent', transition: 'all .3s' }}>
                {i < currentIdx ? '✓' : i + 1}
              </div>
              <div style={{ fontSize: 9, color: i <= currentIdx ? '#7c3aed' : '#c4b5fd', fontWeight: 600, textAlign: 'center', lineHeight: 1.3 }}>
                {s.key === 'collecting' ? 'Collect' : s.key === 'compressing' ? 'Compress' : s.key === 'downloading' ? 'Download' : 'Done'}
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Photo count badge */}
        {stage !== 'done' && (
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(155,127,232,0.1)', border: '1px solid rgba(155,127,232,0.2)', padding: '6px 16px', borderRadius: 20 }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#9b7fe8', animation: 'pulse 1.5s ease infinite' }} />
            <span style={{ fontSize: 12, fontWeight: 700, color: '#7c3aed' }}>{total} photos</span>
          </div>
        )}

        {/* Done state — close button */}
        {stage === 'done' && (
          <button onClick={onClose} style={{ padding: '12px 32px', borderRadius: 14, background: 'linear-gradient(135deg,#9b7fe8,#c084fc)', color: '#fff', border: 'none', fontSize: 14, fontWeight: 700, cursor: 'pointer', boxShadow: '0 6px 20px rgba(155,127,232,0.4)' }}>
            ✓ Done
          </button>
        )}
      </div>
...
// ─── Folder Picker Modal ──────────────────────────────────────────────────────
function FolderPickerModal({ count, eventName, onChooseFolder, onUseDefault, onCancel }) {
  const supported = typeof window !== 'undefined' && 'showDirectoryPicker' in window

  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(13,13,26,0.85)', backdropFilter: 'blur(20px)', fontFamily: "'Inter',system-ui,sans-serif" }}>
      <style>{ANIM}</style>
      <div style={{ background: 'rgba(255,255,255,0.96)', borderRadius: 28, padding: '36px 40px', maxWidth: 420, width: '90%', boxShadow: '0 32px 80px rgba(45,27,105,0.25)', textAlign: 'center', animation: 'zoomIn .25s ease' }}>

        <div style={{ fontSize: 48, marginBottom: 16 }}>📁</div>
        <div style={{ fontSize: 18, fontWeight: 800, color: '#2d1b69', marginBottom: 8 }}>
          Download {count} Photos
        </div>
        <div style={{ fontSize: 13, color: '#9b89c4', marginBottom: 28, lineHeight: 1.6 }}>
          Photos will be downloaded as a ZIP file.<br />
          {supported ? 'Where would you like to save them?' : 'Your file will be saved to your downloads folder.'}
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {/* Folder picker — Chrome only */}
          {supported && (
            <button onClick={onChooseFolder} style={{ width: '100%', padding: '14px', borderRadius: 16, background: 'linear-gradient(135deg,#9b7fe8,#c084fc)', color: '#fff', border: 'none', fontSize: 14, fontWeight: 700, cursor: 'pointer', boxShadow: '0 6px 20px rgba(155,127,232,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              📂 Choose Folder
            </button>
          )}

          {/* Default downloads */}
          <button onClick={onUseDefault} style={{ width: '100%', padding: '14px', borderRadius: 16, background: supported ? 'rgba(255,255,255,0.7)' : 'linear-gradient(135deg,#9b7fe8,#c084fc)', color: supported ? '#7c3aed' : '#fff', border: supported ? '1.5px solid rgba(155,127,232,0.25)' : 'none', fontSize: 14, fontWeight: 700, cursor: 'pointer', boxShadow: supported ? 'none' : '0 6px 20px rgba(155,127,232,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ⬇ {supported ? 'Use Default Downloads' : 'Download ZIP'}
          </button>

          <button onClick={onCancel} style={{ width: '100%', padding: '10px', borderRadius: 12, background: 'transparent', color: '#c4b5fd', border: 'none', fontSize: 13, cursor: 'pointer' }}>
            Cancel
          </button>
        </div>
      </div>
...
function CameraPage({ onCapture, onClose }) {
  ...
  async function startCamera() {
    stopCamera(); setError(''); setReady(false)
    try {
    ...
    } catch (e) { if (mountedRef.current) setError('Camera access denied. Please allow camera permission.') }
  }
  ...
  function capturePhoto() {
    setShutter(true); setTimeout(() => setShutter(false), 500)
  ...
  function retake() { setCaptured(null); setReady(false); startCamera() }
  function confirm() { canvasRef.current.toBlob(blob => onCapture(blob), 'image/jpeg', 0.9) }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: '#0d0d1a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', opacity: visible ? 1 : 0, transition: 'opacity .3s', fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 28, animation: 'fadeUp .4s ease' }}>
        ...
        <p style={{ fontSize: 13, color: 'rgba(192,132,252,0.9)', fontWeight: 600 }}>{error || 'Keep face in center · Look straight at camera'}</p>
      </div>
          ...
          <>
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 4 }}>
              ...
              <button onClick={startCountdown} disabled={!ready || countdown !== null}
                style={{ width: 68, height: 68, borderRadius: '50%', background: ready ? 'linear-gradient(135deg,#9b7fe8,#c084fc)' : 'rgba(155,127,232,0.3)', border: '4px solid rgba(255,255,255,0.25)', cursor: ready ? 'pointer' : 'not-allowed', fontSize: 24, boxShadow: ready ? '0 0 30px rgba(155,127,232,0.7)' : 'none', transition: 'all .2s', position: 'relative', zIndex: 2 }}>📷</button>
            </div>
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginTop: 0 }}>3-second countdown · Look straight</p>
            <button onClick={() => setFacingMode(f => f === 'user' ? 'environment' : 'user')} style={{ padding: '9px 22px', borderRadius: 20, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.6)', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>🔄 Flip Camera</button>
          </>
        ) : (
...
function ImageCard({ photo, index, selected, onSelect, onFullView, onDownload }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onDoubleClick={onFullView}
      style={{ gridColumn: isLarge ? 'span 2' : 'span 1', position: 'relative', borderRadius: 18, overflow: 'hidden', border: `2px solid ${selected ? '#9b7fe8' : 'transparent'}`, boxShadow: selected ? '0 0 0 3px rgba(155,127,232,0.25)' : hovered ? '0 16px 40px rgba(155,127,232,0.2)' : '0 4px 16px rgba(0,0,0,0.07)', transition: 'all .22s cubic-bezier(.4,0,.2,1)', transform: hovered ? 'translateY(-3px)' : 'translateY(0)', cursor: 'pointer', aspectRatio: isLarge ? '16/9' : '4/3', animation: `slideUp .4s ease ${Math.min(index * 0.05, 0.5)}s both` }}>
      <img src={photo.url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform .3s cubic-bezier(.4,0,.2,1)', transform: hovered ? 'scale(1.06)' : 'scale(1)' }} onError={e => { e.target.src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="200" height="160"><rect fill="%239b7fe8" opacity=".2" width="200" height="160"/></svg>' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top,rgba(45,27,105,0.85) 0%,rgba(45,27,105,0.1) 60%,transparent 100%)', opacity: hovered ? 1 : 0, transition: 'opacity .22s' }}>
        <div style={{ position: 'absolute', bottom: 12, left: 12, right: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {photo.similarity && <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.9)', fontWeight: 700, background: 'rgba(155,127,232,0.55)', padding: '2px 8px', borderRadius: 20 }}>{Math.round(photo.similarity * 100)}% match</span>}
          <button onClick={e => { e.stopPropagation(); onDownload() }} style={{ marginLeft: 'auto', padding: '6px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.95)', color: '#7c3aed', border: 'none', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>⬇ Save</button>
        </div>
      </div>
      <div onClick={e => { e.stopPropagation(); onSelect() }} style={{ position: 'absolute', top: 10, left: 10, width: 26, height: 26, borderRadius: 8, background: selected ? 'linear-gradient(135deg,#9b7fe8,#c084fc)' : 'rgba(255,255,255,0.92)', border: `2px solid ${selected ? 'transparent' : 'rgba(155,127,232,0.3)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', opacity: hovered || selected ? 1 : 0, transition: 'all .15s', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
...
// ─── Page 3: Gallery ──────────────────────────────────────────────────────────
function GalleryPage({ photos, guestName, onFaceSearch, searching, slug, event }) {
  ...
  const [zipStage, setZipStage] = useState(null) // null | 'collecting' | 'compressing' | 'downloading' | 'done'
  ...
  async function downloadOne(url, name) {
    ...
    const proxyUrl = `${API_URL}/api/search/download?url=${encodeURIComponent(url)}&filename=${encodeURIComponent(filename)}`
  ...
  // ── Step 1: User clicks Download Selected → show folder picker ───────────
  function handleDownloadClick() {
    if (selected.size === 0) return
    setShowFolderPicker(true)
  }

  // ── Step 2a: User picks a folder (Chrome only) ────────────────────────────
  async function downloadToFolder() {
    setShowFolderPicker(false)
    await performZipDownload(true)
  }

  // ── Step 2b: User uses default downloads folder ───────────────────────────
  async function downloadToDefault() {
    setShowFolderPicker(false)
    await performZipDownload(false)
  }

  // ── Core ZIP download with staged animation ───────────────────────────────
  async function performZipDownload(useFolder) {
    setZipStage('collecting')

    try {
      const photoIds = Array.from(selected)

      // Stage 1 — collecting (show for at least 600ms so user sees it)
      await new Promise(r => setTimeout(r, 600))
      setZipStage('compressing')

      // Actually fetch the ZIP from backend
      const response = await fetch(`${API_URL}/api/search/${slug}/download-zip`, {
        method: 'POST',
      ...
      const zipName = `${event?.name || 'photos'}_${photoIds.length}_photos.zip`

      if (useFolder && 'showDirectoryPicker' in window) {
        ...
        try {
          ...
          // User cancelled folder picker — fall back to default download
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#ddd6fe 0%,#e9d5ff 20%,#fbcfe8 50%,#fde68a 80%,#ddd6fe 100%)', fontFamily: "'Inter',system-ui,sans-serif", position: 'relative' }}>
      ...
      {showFolderPicker && (
        <FolderPickerModal
          ...
          onChooseFolder={downloadToFolder}
          onUseDefault={downloadToDefault}
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto', height: 58, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {selected.size > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 12px', background: 'rgba(155,127,232,0.1)', border: '1px solid rgba(155,127,232,0.2)', borderRadius: 20, animation: 'zoomIn .2s ease' }}>
                ...
                <button onClick={clearSel} style={{ padding: '4px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.6)', color: '#9b89c4', border: '1px solid rgba(155,127,232,0.2)', fontSize: 11, cursor: 'pointer' }}>✕</button>
              </div>
            )}
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 20px' }}>
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            {searching ? (
              ...
              <>
                ...
                <p style={{ fontSize: 13, color: '#9b89c4', marginTop: 6 }}>Double-click any photo to view full size · Select photos to download as ZIP</p>
              </>
            )}
          </div>
          ...
          {!searching && photos.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <button onClick={onFaceSearch} style={{ padding: '13px 28px', borderRadius: 14, background: 'linear-gradient(135deg,#9b7fe8,#c084fc)', color: '#fff', border: 'none', fontSize: 14, fontWeight: 700, cursor: 'pointer', boxShadow: '0 6px 20px rgba(155,127,232,0.4)' }}>🔍 Try Again</button>
            </div>
          )}
          ...
          {photos.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
              {photos.map((photo, i) => (
                <ImageCard key={photo.photo_id || i} photo={photo} index={i} selected={selected.has(photo.photo_id)} onSelect={() => toggle(photo.photo_id)} onFullView={() => openModal(photo, i)} onDownload={() => downloadOne(photo.url, `photo-${i + 1}.jpg`)} />
              ))}
            </div>
          )}
        </div>
      ...
      {/* Lightbox */}
      {fullView && (
        <div onClick={() => setFullView(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(8,8,18,0.97)', backdropFilter: 'blur(28px)', zIndex: 200, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 60px', animation: 'zoomIn .2s ease' }}>
          <button onClick={() => setFullView(null)} style={{ position: 'absolute', top: 20, right: 20, width: 42, height: 42, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>✕</button>
          <div style={{ position: 'absolute', top: 24, left: '50%', transform: 'translateX(-50%)', fontSize: 12, color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>{fullViewIdx + 1} / {photos.length}</div>
          <button onClick={e => { e.stopPropagation(); prevPhoto() }} disabled={fullViewIdx === 0} style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', width: 44, height: 44, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', fontSize: 20, cursor: fullViewIdx === 0 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: fullViewIdx === 0 ? 0.3 : 1, zIndex: 10 }}>‹</button>
          <div onClick={e => e.stopPropagation()} style={{ animation: 'zoomIn .25s ease' }}>
            <img src={fullView.url} alt="" style={{ maxWidth: 'min(88vw, 1000px)', maxHeight: '78vh', objectFit: 'contain', borderRadius: 16, boxShadow: '0 32px 80px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.05)', display: 'block' }} />
          </div>
          <button onClick={e => { e.stopPropagation(); nextPhoto() }} disabled={fullViewIdx === photos.length - 1} style={{ position: 'absolute', right: 16, top: '50%', transform: 'translateY(-50%)', width: 44, height: 44, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', fontSize: 20, cursor: fullViewIdx === photos.length - 1 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: fullViewIdx === photos.length - 1 ? 0.3 : 1, zIndex: 10 }}>›</button>
          <div onClick={e => e.stopPropagation()} style={{ marginTop: 18, display: 'flex', gap: 10 }}>
            <button onClick={() => downloadOne(fullView.url, `photo-${fullViewIdx + 1}.jpg`)} style={{ padding: '10px 24px', borderRadius: 12, background: 'linear-gradient(135deg,#9b7fe8,#c084fc)', color: '#fff', border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 16px rgba(155,127,232,0.5)' }}>⬇ Download</button>
            <button onClick={() => setFullView(null)} style={{ padding: '10px 20px', borderRadius: 12, background: 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.15)', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Close</button>
          </div>
        </div>
      )}
    </div>
...
export default function GuestPage() {
  ...
  async function handleCapture(blob) {
    ...
    try {
      ...
      if (data.matches) persistPhotos(data.matches)
      else setError(data.detail || 'No faces found. Try again.')
  ...
  return <GalleryPage photos={photos} guestName={guestInfo?.name || 'Guest'} searching={searching} onFaceSearch={() => persistPage('camera')} slug={slug} event={event} />
}